...
INTENTS = ("location", "directions", "calendar", "general")
...
class Assistant:
    ...
    def _classify_keywords(text: str) -> str | None:
        t = text.lower()
        if re.search(r"\b(directions?|navigate|take me to|how (do|can) i get to|"
                     r"how far (is|to)|route to|the way to|walk to|get to)\b", t):
            return "directions"
    ...
    def _classify_gemini(self, text: str) -> str | None:
        ...
        try:
            ...
            prompt = (
                ...
                "location, directions, calendar, or general.\n"
                "- location: asking where they are, their address, surroundings.\n"
                "- directions: asking how to get somewhere, or distance/route to a place.\n"
    ...
    def handle(self, text: str) -> str:
        ...
        if intent == "directions":
            return self.location.directions(self._extract_destination(text))
        if intent == "calendar":
    ...
    def _extract_destination(text: str) -> str:
        """Strip lead-in phrases so only the place name is left for geocoding."""
        return re.sub(
            r"(?i)^(please\s+)?(can you\s+)?(give me\s+)?"
            r"(directions?\s+(to|for)\s+|how (do|can) i get to\s+|"
            r"navigate\s+(me\s+)?to\s+|take me to\s+|route to\s+|"
            r"the way to\s+|walk to\s+|get to\s+|how far (is|to)\s+(the\s+)?)",
            "", text.strip(),
        ).strip(" ?.")
