...
import requests
import json
# The Shaped SDK class name varies by version (legacy: ShapedClient; current
# 2.x: Client). Import resiliently so a version skew never crashes module load.
try:
...
# WEAVE_PROJECT is overridable so each teammate can point traces at their own
# W&B entity (Nghia's hardcoded entity → permission-denied for others).
WEAVE_PROJECT = os.environ.get(
    "WEAVE_PROJECT", "nghiatr38-boston-university/project-unleash-service-dog"
)
try:
...
# Build the heavy clients fail-soft: importing this module must NEVER raise
# (ui.py / feat_ui.py import it at module load, before any tab's try/except runs).
# With missing keys each constructor can throw; we degrade to None and every agent
# below tolerates it, so the dashboard still renders and runs in Safe mode.
try:
    memory_client = Memory.from_config(mem0_config)
except Exception as e:
    memory_client = None
    print(f"[mem0] memory disabled ({type(e).__name__}); set GROQ_API_KEY (+ embeddings deps) to enable.")

# Groq client for fast Llama-3 orchestration
try:
    ai_client = Groq()
except Exception as e:
    ai_client = None
    print(f"[groq] LLM disabled ({type(e).__name__}); set GROQ_API_KEY to enable.")

# Shaped SDK for ranked episodic recall
try:
    shaped_client = ShapedClient(api_key=os.environ.get("SHAPED_API_KEY")) if ShapedClient else None
    if shaped_client is None:
        print("[shaped] SDK class not found (version skew) or SHAPED_API_KEY unset; recall disabled.")
except Exception as e:
    shaped_client = None
    print(f"[shaped] memory recall disabled ({type(e).__name__}); set SHAPED_API_KEY to enable.")
...
def memory_agent(state: ConflictBusState) -> dict:
    ...
    if not predictions or shaped_client is None:
