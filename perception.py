...
def parse_args():
    parser = argparse.ArgumentParser(description="Service dog perception agent")
...
def main():
    ...
    if not cap.isOpened():
        print(f"ERROR: Cannot open camera {args.camera}. Run list_cams.py.",
              file=sys.stderr)
        sys.exit(1)

    W = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    H = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    print(f"Camera open: {W}x{H}. Press 'q' to quit.", file=sys.stderr)
    ...
    while True:
        ...
        cv2.imshow("service dog view", frame)
